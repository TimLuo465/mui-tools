...
function compile(modules) {
  ...
  const assets = gulp.src(['components/**/*.@(|png|svg|woff|woff2|ttf|eot)']).pipe(gulp.dest(modules === false ? esDir : libDir));
  let error = 0;
  const source = [
    'components/**/*.tsx',
    'components/**/*.ts',
...
function publish(tagString, done) {
  let args = ['publish', '--with-mui-tools', '--access=public'];
  if (tagString) {
...
gulp.task('guard', (done) => {
  ...
  if (npmArgs) {
    for (let arg = npmArgs.shift(); arg; arg = npmArgs.shift()) {
      if (/^pu(b(l(i(sh?)?)?)?)?$/.test(arg) && npmArgs.indexOf('--with-mui-tools') < 0) {
        reportError();
        done(1);
        return;
      }
